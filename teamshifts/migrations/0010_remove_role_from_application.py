...
def cleanup_legacy_order_role(apps, schema_editor):
    CallForTeamMembers = apps.get_model("teamshifts", "CallForTeamMembers")
    for cfm in CallForTeamMembers.objects.all():
        if isinstance(cfm.field_order, list) and "role" in cfm.field_order:
            cfm.field_order = [item for item in cfm.field_order if item != "role"]
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(dedupe_team_member_applications, migrations.RunPython.noop),
        migrations.RunPython(cleanup_legacy_order_role, migrations.RunPython.noop),
